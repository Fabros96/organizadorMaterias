...
# Diccionario para normalizar los días ingresados
dias_validos = {
    "lunes": "Lunes",
    "martes": "Martes",
    "miercoles": "Miércoles",
...
class HorarioGUI:
    ...
    def menu_abm_horarios(self):
        ...
        def agregar_horario():
            materia = combo_materia.get()
            comision = combo_comision.get()
            dia = entry_dia.get().strip().lower()

            # Normalizar día
            dia = dias_validos.get(dia, None)
            if not dia:
                messagebox.showerror("Error", "Día inválido")
                return

            # Obtener horas y minutos
            horasDesde = entry_horas_desde.get().strip() or "00"
            minutosDesde = entry_minutos_desde.get().strip() or "00"
            horasHasta = entry_horas_hasta.get().strip() or "00"
            minutosHasta = entry_minutos_hasta.get().strip() or "00"

            # Validar rangos
            if not (0 <= int(horasDesde) <= 23) or not (0 <= int(horasHasta) <= 23):
                messagebox.showerror("Error", "Las horas deben estar entre 00 y 23")
                return
            ...
            # Agregar horario
            if materia and comision:
                materias[materia][comision].append((dia, hora_decimal_desde, hora_decimal_hasta))
                actualizar_horarios()
                messagebox.showinfo("Éxito", f"Horario agregado: {dia} {horasDesde}:{minutosDesde}-{horasHasta}:{minutosHasta}")
            else:
                messagebox.showerror("Error", "Datos inválidos")
    ...
    def importar_json(self):
        ...
        if file_path:
            try:
                ...
                # Normalizar los días en todos los horarios
                for materia, comisiones in materias.items():
                    for comision, horarios in comisiones.items():
                        for i, (dia, inicio, fin) in enumerate(horarios):
                            dia_norm = dias_validos.get(dia.lower(), dia)
                            horarios[i] = (dia_norm, inicio, fin)
    ...
    def crear_menu_principal(self):
        ...
        for texto, comando in [
            ...
            # ("Select Plan", self.menu_abm_horarios), # Placeholder para futura funcionalidad hacer que elijan un plan de estudio y luego seleccione las materias que quiere
        ]:
